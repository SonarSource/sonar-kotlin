...
val COLLECTION_SIZE_METHOD = FunMatcher(definingSupertype = "kotlin.collections.Collection", name = "size") {
    withNoArguments()
}
...
class CollectionSizeAndArrayLengthCheck : AbstractCheck() {
    ...
    private fun checkConditionsAndSelectMessage(
        ...
        if (testedExpr is KtDotQualifiedExpression && MATCHERS.any { it.matches(testedExpr.resolveToCall()?.singleVariableAccessCall()) }) {
            if (opToken == opWithEq) {
