...
class DeprecatedCodeCheck : AbstractCheck() {
    ...
    override fun visitAnnotationEntry(annotationEntry: KtAnnotationEntry, context: KotlinFileContext) {
        val annotated: KtAnnotated = annotationEntry.annotatedElement()
        if (annotated !is KtDeclaration) return
        analyze(annotated) {
            if (ClassId.fromString("kotlin/Deprecated") in annotated.symbol.annotations) {
                context.reportIssue(
